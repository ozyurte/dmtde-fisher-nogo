...
int background_functions(
 ...
 /* cold dark matter with dynamic DMTDE */
  if (pba->has_cdm == _TRUE_) {

    double rho_cdm_LCDM = pba->Omega0_cdm * pow(pba->H0,2) / pow(a,3);

    if (pba->use_dmtde == _TRUE_) {

      double ln_a  = log(a);
      double ln_ac = -log(1.0 + pba->dmtde_zc);
      double s     = pba->dmtde_dlnac;

      /* f_d(a): Gaussian transition */
      double fd = pba->dmtde_fd0 *
                  exp(-pow(ln_a - ln_ac, 2) / (2.0 * s * s));

      /* modified CDM density */
      pvecback[pba->index_bg_rho_cdm] = rho_cdm_LCDM * (1.0 - fd);

      /* extra dark energy component (w = -1) */
      double rho_de_extra = rho_cdm_LCDM * fd;

      /* add to totals */
      rho_tot += rho_de_extra;
      p_tot   -= rho_de_extra;
    }
    else {
      pvecback[pba->index_bg_rho_cdm] = rho_cdm_LCDM;
    }
