...
int input_read_parameters_species(struct file_content * pfc,
  ...
  /* ----------------------------------------------------
     DMTDE EXTENSION PARAMETERS
     ---------------------------------------------------- */

  /* use_dmtde flag (0/1) */
  class_call(parser_read_int(pfc,"use_dmtde",&flag2,&flag1,errmsg),
             errmsg,
             errmsg);
  if (flag1 == _TRUE_) {
    pba->use_dmtde = (short)flag2;
  }
  else {
    pba->use_dmtde = _FALSE_; /* default: off */
  }

  /* dmtde_fd0: present-day dissolved DM fraction */
  class_call(parser_read_double(pfc,"dmtde_fd0",&param1,&flag1,errmsg),
             errmsg,
             errmsg);
  pba->dmtde_fd0 = (flag1 == _TRUE_) ? param1 : 0.05;

  /* dmtde_zc: transition redshift */
  class_call(parser_read_double(pfc,"dmtde_zc",&param1,&flag1,errmsg),
             errmsg,
             errmsg);
  pba->dmtde_zc = (flag1 == _TRUE_) ? param1 : 1.5;

  /* dmtde_dlnac: width in ln(a) */
  class_call(parser_read_double(pfc,"dmtde_dlnac",&param1,&flag1,errmsg),
             errmsg,
             errmsg);
  pba->dmtde_dlnac = (flag1 == _TRUE_) ? param1 : 0.1;
